...
class gradient:
	...
	def get(self,ll,return_G=False):
		...
		(self.dlnv_e_rho,self.dlnv_e_lambda,self.dlnv_e_beta)=(dlnv_e_rho,dlnv_e_lambda,dlnv_e_beta)
		(self.dlnv_e_rho_G,self.dlnv_e_lambda_G,self.dlnv_e_beta_G)=(dlnv_e_rho_G,dlnv_e_lambda_G,dlnv_e_beta_G)
		...
		#final derivatives:
		dLL_beta=rp.add((rp.prod((dlnv_e_beta,dLL_lnv)),rp.prod((self.de_beta_RE,DLL_e))),True)
		dLL_rho=rp.add((rp.prod((dlnv_e_rho,dLL_lnv)),rp.prod((self.de_rho_RE,DLL_e))),True)
		dLL_lambda=rp.add((rp.prod((dlnv_e_lambda,dLL_lnv)),rp.prod((self.de_lambda_RE,DLL_e))),True)
...
class hessian:
	...
	def hessian(self,ll):
		...
		d2lnv_gamma2		=   rp.dd_func_lags(panel,ll,GARK, 	g_obj.dlnv_gamma,						g_obj.dLL_lnv,  transpose=True)
		d2lnv_gamma_psi		=	rp.dd_func_lags(panel,ll,GARK, 	g_obj.dlnv_psi,							g_obj.dLL_lnv)

		d2lnv_gamma_rho		=	rp.dd_func_lags(panel,ll,GARK,	g_obj.dlnv_e_rho_G,						g_obj.dLL_lnv)
		d2lnv_gamma_lambda	=	rp.dd_func_lags(panel,ll,GARK, 	g_obj.dlnv_e_lambda_G,					g_obj.dLL_lnv)
		d2lnv_gamma_beta	=	rp.dd_func_lags(panel,ll,GARK, 	g_obj.dlnv_e_beta_G,					g_obj.dLL_lnv)
		d2lnv_gamma_z		=	rp.dd_func_lags(panel,ll,GARK, 	g_obj.dlnv_z_G,							g_obj.dLL_lnv)

		d2lnv_psi_rho		=	rp.dd_func_lags(panel,ll,GARM, 	rp.prod((ll.h_e_val,g_obj.de_rho)),		g_obj.dLL_lnv)
		d2lnv_psi_lambda	=	rp.dd_func_lags(panel,ll,GARM, 	rp.prod((ll.h_e_val,g_obj.de_lambda)),	g_obj.dLL_lnv)
		d2lnv_psi_beta		=	rp.dd_func_lags(panel,ll,GARM, 	rp.prod((ll.h_e_val,g_obj.de_beta)),	g_obj.dLL_lnv)
		d2lnv_psi_z			=	rp.dd_func_lags(panel,ll,GARM, 	ll.h_z_val,								g_obj.dLL_lnv)

		AMAq=-rp.ARMA_product(ll.AMA_1,panel.q)
		d2lnv_lambda2,		d2e_lambda2		=	rp.dd_func_lags_mult(panel,ll,g_obj,AMAq,	g_obj.de_lambda,	g_obj.de_lambda,	'lambda',	'lambda', transpose=True)
		d2lnv_lambda_rho,	d2e_lambda_rho	=	rp.dd_func_lags_mult(panel,ll,g_obj,AMAq,	g_obj.de_lambda,	g_obj.de_rho,		'lambda',	'rho' )
		d2lnv_lambda_beta,	d2e_lambda_beta	=	rp.dd_func_lags_mult(panel,ll,g_obj,AMAq,	g_obj.de_lambda,	g_obj.de_beta,		'lambda',	'beta')

		AMAp=-rp.ARMA_product(ll.AMA_1,panel.p)
		d2lnv_rho_beta,		d2e_rho_beta	=	rp.dd_func_lags_mult(panel,ll,g_obj,AMAp,	g_obj.de_rho,		g_obj.de_beta,		'rho',		'beta', de_zeta_u=-panel.X)
		
		d2lnv_mu_rho,d2lnv_mu_lambda,d2lnv_mu_beta,d2lnv_mu_z,mu=None,None,None,None,None
		if panel.N>1:
			d2lnv_mu_rho			=	rp.dd_func_lags(panel,ll,None, 		rp.prod((ll.h_e_val,g_obj.de_rho)),		g_obj.dLL_lnv, 	addavg=1) 
			d2lnv_mu_lambda			=	rp.dd_func_lags(panel,ll,None, 		rp.prod((ll.h_e_val,g_obj.de_lambda)),	g_obj.dLL_lnv, 	addavg=1) 
			d2lnv_mu_beta			=	rp.dd_func_lags(panel,ll,None, 		rp.prod((ll.h_e_val,g_obj.de_beta)),	g_obj.dLL_lnv, 	addavg=1) 
			d2lnv_mu_z				=	rp.dd_func_lags(panel,ll,None, 		ll.h_z_val,								g_obj.dLL_lnv, 	addavg=1) 
			mu=ll.args_d['mu']
	
		d2lnv_z2				=	rp.dd_func_lags(panel,ll,ll.GAR_1MA, ll.h_2z_val,							g_obj.dLL_lnv, 	addavg=mu) 
		d2lnv_z_rho				=	rp.dd_func_lags(panel,ll,ll.GAR_1MA, rp.prod((ll.h_ez_val,g_obj.de_rho)),	g_obj.dLL_lnv, 	addavg=mu) 
		d2lnv_z_lambda			=	rp.dd_func_lags(panel,ll,ll.GAR_1MA, rp.prod((ll.h_ez_val,g_obj.de_lambda)),g_obj.dLL_lnv, 	addavg=mu) 
		d2lnv_z_beta			=	rp.dd_func_lags(panel,ll,ll.GAR_1MA, rp.prod((ll.h_ez_val,g_obj.de_beta)),	g_obj.dLL_lnv, 	addavg=mu) 
		
		d2lnv_rho2,	d2e_rho2	=	rp.dd_func_lags_mult(panel,ll,g_obj,	None,	g_obj.de_rho,		g_obj.de_rho,		'rho',		'rho' )
		d2lnv_beta2,d2e_beta2	=	rp.dd_func_lags_mult(panel,ll,g_obj,	None,	g_obj.de_beta,		g_obj.de_beta,		'beta',		'beta')
		...
		(de_rho_RE,de_lambda_RE,de_beta_RE)=(g_obj.de_rho_RE,g_obj.de_lambda_RE,g_obj.de_beta_RE)
		(dlnv_e_rho,dlnv_e_lambda,dlnv_e_beta)=(g_obj.dlnv_e_rho,g_obj.dlnv_e_lambda,g_obj.dlnv_e_beta)
		(dlnv_gamma,dlnv_psi)=(g_obj.dlnv_gamma,g_obj.dlnv_psi)
		(dlnv_mu,dlnv_z)=(g_obj.dlnv_mu, g_obj.dlnv_z)		
		...
		D2LL_beta_gamma		=	rp.dd_func(None,		d2LL_dln_de,	d2LL_dln2,	de_beta_RE, 	None,			dlnv_e_beta, 	g_obj.dlnv_gamma,		None, 					T(d2lnv_gamma_beta))
		D2LL_beta_psi		=	rp.dd_func(None,		d2LL_dln_de,	d2LL_dln2,	de_beta_RE, 	None,			dlnv_e_beta, 	g_obj.dlnv_psi,		None, 					T(d2lnv_psi_beta))
		...
		D2LL_rho_gamma		=	rp.dd_func(None,		d2LL_dln_de,	d2LL_dln2,	de_rho_RE, 		None,			dlnv_e_rho, 	g_obj.dlnv_gamma,		None, 					T(d2lnv_gamma_rho))	
		D2LL_rho_psi		=	rp.dd_func(None,		d2LL_dln_de,	d2LL_dln2,	de_rho_RE, 		None,			dlnv_e_rho, 	g_obj.dlnv_psi,		None, 					T(d2lnv_psi_rho))
		...
		D2LL_lambda_gamma	=	rp.dd_func(None,		d2LL_dln_de,	d2LL_dln2,	de_lambda_RE, 	None,			dlnv_e_lambda, 	g_obj.dlnv_gamma,		None, 					T(d2lnv_gamma_lambda))
		D2LL_lambda_psi		=	rp.dd_func(None,		d2LL_dln_de,	d2LL_dln2,	de_lambda_RE, 	None,			dlnv_e_lambda, 	g_obj.dlnv_psi,		None, 					T(d2lnv_psi_lambda))
		...
		D2LL_gamma2			=	rp.dd_func(None,		None,			d2LL_dln2,	None, 			None,			g_obj.dlnv_gamma, 	g_obj.dlnv_gamma,		None, 					T(d2lnv_gamma2))
		D2LL_gamma_psi		=	rp.dd_func(None,		None,			d2LL_dln2,	None,			None,			g_obj.dlnv_gamma, 	g_obj.dlnv_psi,		None, 					d2lnv_gamma_psi)
		D2LL_gamma_omega	=	rp.dd_func(None,		None,			d2LL_dln2,	None, 			None,			g_obj.dlnv_gamma, 	panel.W_a,		None, 					None)
		D2LL_gamma_mu		=	rp.dd_func(None,		None,			d2LL_dln2,	None, 			None,			g_obj.dlnv_gamma, 	dlnv_mu,		None, 					None)
		D2LL_gamma_z		=	rp.dd_func(None,		None,			d2LL_dln2,	None, 			None,			g_obj.dlnv_gamma, 	dlnv_z,			None, 					d2lnv_gamma_z)
		
		D2LL_psi2			=	rp.dd_func(None,		None,			d2LL_dln2,	None,			None,			g_obj.dlnv_psi, 		g_obj.dlnv_psi,		None, 					None)
		D2LL_psi_omega		=	rp.dd_func(None,		None,			d2LL_dln2,	None, 			None,			g_obj.dlnv_psi, 		panel.W_a,		None, 					None)
		D2LL_psi_mu			=	rp.dd_func(None,		None,			d2LL_dln2,	None, 			None,			g_obj.dlnv_psi, 		dlnv_mu,		None, 					None)
		D2LL_psi_z			=	rp.dd_func(None,		None,			d2LL_dln2,	None, 			None,			g_obj.dlnv_psi, 		dlnv_z,			None, 					d2lnv_psi_z)
	...
	def second_derivatives_mp(self,ll,mp,g_obj):
		...
		(d['de_rho_RE'],d['de_lambda_RE'],d['de_beta_RE'])=(g_obj.de_rho_RE,g_obj.de_lambda_RE,g_obj.de_beta_RE)
		(d['dlnv_e_rho'],d['dlnv_e_lambda'],d['dlnv_e_beta'])=(g_obj.dlnv_e_rho,g_obj.dlnv_e_lambda,g_obj.dlnv_e_beta)
		(d['dlnv_gamma'],d['dlnv_psi'])=(g_obj.dlnv_gamma,g_obj.dlnv_psi)

		(d['dlnv_mu'], d['dlnv_z'])=(g_obj.dlnv_mu, g_obj.dlnv_z)		
...
class g_obj_light():
	def __init__(self,g_obj):
		"""A minimalistic version of g_obj object for multiprocessing. Reduces the amount of information 
			transfered to the nodes"""

		self.DLL_e			=	g_obj.DLL_e
		self.dLL_lnv		=	g_obj.dLL_lnv
		self.dlnv_gamma		=	g_obj.dlnv_gamma
		self.dlnv_psi		=	g_obj.dlnv_psi
		self.dlnv_e_rho_G	=	g_obj.dlnv_e_rho_G
		self.dlnv_e_lambda_G=	g_obj.dlnv_e_lambda_G
		self.dlnv_e_beta_G	=	g_obj.dlnv_e_beta_G
		self.dlnv_z_G		=	g_obj.dlnv_z_G
		self.de_lambda		=	g_obj.de_lambda
		self.de_rho			=	g_obj.de_rho
