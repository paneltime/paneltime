...
def add_initial_constraints(constr,panel,user_constraints,ll,its):
	...
	add_constraints_list(constr,user_constraints,ll)
	general_constraints=[('rho',-2,2),('lambda',-2,2),('gamma',-2,2),('psi',-2,2)]
	add_constraints_list(constr,general_constraints,ll)
	if panel.m_zero and panel.loadargs==False:
		if its<1:
			constr.add(panel.args.map_from_categories['rho'][0],None,'initial rho constr',value=0.3)
		if its<3:
			constr.add(panel.args.map_from_categories['gamma'][0],None,'initial gamma constr',value=0.1)
		constr.add(panel.args.map_from_categories['psi'][0],None,'psi=0 constr',value=0.1)
...
def add_constraints_list(constr,constraints,ll):
	...
	for i in range(2):
		if type(m[i])==str:
			if not ll is None:
				...
				return
	[minimum,maximum]=m
	constr.set_named(name,None,'user constraint', [minimum,maximum])
...
class constraints:
	...
	def add(self,index,assco,cause,interval=None,replace=True,value=None):
		...
		category,j=self.panel.args.map_to_categories[index]
	...
	def set_named(self,name,assco,cause,interval):
