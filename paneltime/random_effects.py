...
class re_obj:
	...
	def RE(self,e,recalc=True):
		...
		if recalc:
			self.eFE=self.FE(e)
			self.vLSDV=np.sum(self.eFE**2)/self.panel.NT
			N,T,k=e.shape
			e_mean=np.sum(e,1)/self.panel.T_i.reshape((N,1))
			self.grp_v=np.sum(e_mean**2)/self.panel.N
			self.theta=(1-np.sqrt(self.vLSDV/(self.grp_v*self.panel.T_i)))*self.panel.included
	...
	def dRE(self,de,e,vname):
		...
		if self.panel.FE_RE==0:
			...
			return self.FE(de)	
		sqrt_expr=np.sqrt(1/(self.grp_v*self.panel.T_i*self.vLSDV))
		self.deFE[vname]=self.FE(de)
		self.dvLSDV[vname]=np.sum(np.sum(self.eFE*self.deFE[vname],0),0)/self.panel.NT
	...
	def ddRE(self,dde,de1,de2,e,vname1,vname2):
		...
		ddvLSDV=np.sum(np.sum(eFE*ddeFE+self.deFE[vname1].reshape(N,T,k,1)*self.deFE[vname2].reshape(N,T,1,m),0),0)/self.panel.NT
	
		ddtheta1=(np.sqrt(1/(self.grp_v*self.panel.T_i*(self.vLSDV**2))))*self.dvLSDV[vname1].reshape(k,1)*self.dvLSDV[vname2].reshape(1,m)
		ddtheta2=ddtheta1+(-np.sqrt(1/(self.grp_v*self.panel.T_i*self.vLSDV)))*ddvLSDV
