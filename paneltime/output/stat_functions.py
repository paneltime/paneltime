...
def OLS(panel,X,Y,add_const=False,return_rsq=False,return_e=False,c=None,robust_se_lags=0):
	...
	if return_rsq or return_e or robust_se_lags:
		...
		if return_rsq:
			...
			se_robust,se,V, W=robust_se(panel,robust_se_lags,XXInv,X*e)
...
def robust_se(panel,L,hessin,XErr,nw_only=True):
	...
	nw,NW=sandwich_var(hessin,newey_west_wghts(L,XErr))#newy-west starting at L=1
	if panel.N>1:
	...
	# Since NW and W are calculated starting at L=1, and V is calculated starting at L=0, we can just add them toghether
	V=V+W
	s=np.max(w+v,0)
	se_robust=np.maximum(s,0)**0.5
	i=np.argmax(np.sum(w+v,1))
	se_std=np.maximum(w,0)**0.5
	return se_robust,se_std,V[i], W
