...
class hessian_partials():
	
	def __init__(self,ll,panel,g):

		GARM=rp.ARMA_product(ll.GAR_1,panel.m)
		GARK=rp.ARMA_product(ll.GAR_1,panel.k)

		self.d2lnv_gamma2			=   rp.dd_func_lags(panel,ll,GARK, 	g.dlnv_gamma,						g.dLL_lnv,  transpose=True)
		self.d2lnv_gamma_psi		=	rp.dd_func_lags(panel,ll,GARK, 	g.dlnv_psi,							g.dLL_lnv)

		self.d2lnv_gamma_rho		=	rp.dd_func_lags(panel,ll,GARK,	g.dlnv_sigma_rho_G,						g.dLL_lnv)
		self.d2lnv_gamma_lambda		=	rp.dd_func_lags(panel,ll,GARK, 	g.dlnv_sigma_lambda_G,					g.dLL_lnv)
		self.d2lnv_gamma_beta		=	rp.dd_func_lags(panel,ll,GARK, 	g.dlnv_sigma_beta_G,					g.dLL_lnv)
		self.d2lnv_gamma_z			=	rp.dd_func_lags(panel,ll,GARK, 	g.dlnv_z_G,							g.dLL_lnv)

		self.d2lnv_psi_rho			=	rp.dd_func_lags(panel,ll,GARM, 	rp.prod((ll.h_e_val,g.de_rho)),		g.dLL_lnv)
		self.d2lnv_psi_lambda		=	rp.dd_func_lags(panel,ll,GARM, 	rp.prod((ll.h_e_val,g.de_lambda)),	g.dLL_lnv)
		self.d2lnv_psi_beta			=	rp.dd_func_lags(panel,ll,GARM, 	rp.prod((ll.h_e_val,g.de_beta)),	g.dLL_lnv)
		self.d2lnv_psi_z			=	rp.dd_func_lags(panel,ll,GARM, 	ll.h_z_val,								g.dLL_lnv)

		AMAq=-rp.ARMA_product(ll.AMA_1,panel.q)
		self.d2lnv_lambda2,		self.d2e_lambda2		=	rp.dd_func_lags_mult(panel,ll,g,AMAq,	g.de_lambda,	g.de_lambda,	'lambda',	'lambda', transpose=True)
		self.d2lnv_lambda_rho,	self.d2e_lambda_rho		=	rp.dd_func_lags_mult(panel,ll,g,AMAq,	g.de_lambda,	g.de_rho,		'lambda',	'rho' )
		self.d2lnv_lambda_beta,	self.d2e_lambda_beta	=	rp.dd_func_lags_mult(panel,ll,g,AMAq,	g.de_lambda,	g.de_beta,		'lambda',	'beta')

		AMAp=-rp.ARMA_product(ll.AMA_1,panel.p)
		self.d2lnv_rho_beta,		self.d2e_rho_beta	=	rp.dd_func_lags_mult(panel,ll,g,AMAp,	g.de_rho,		g.de_beta,		'rho',		'beta', de_zeta_u=-panel.X)
		
		d2lnv_mu_rho,d2lnv_mu_lambda,d2lnv_mu_beta,d2lnv_mu_z=None,None,None,None,None
		if panel.N>1:
			self.d2lnv_mu_rho				=	rp.dd_func_lags(panel,ll,None, 		rp.prod((ll.davg_lne2,g.de_rho)),		g.dLL_lnv, 	addavg=1) 
			self.d2lnv_mu_lambda			=	rp.dd_func_lags(panel,ll,None, 		rp.prod((ll.davg_lne2,g.de_lambda)),	g.dLL_lnv, 	addavg=1) 
			self.d2lnv_mu_beta				=	rp.dd_func_lags(panel,ll,None, 		rp.prod((ll.davg_lne2,g.de_beta)),		g.dLL_lnv, 	addavg=1) 
			self.d2lnv_mu_z=None
			if ll.zmu:
				self.d2lnv_mu_z				=	rp.dd_func_lags(panel,ll,None, 		ll.h_z_val,								g.dLL_lnv, 	addavg=1)
	
		self.d2lnv_z2					=	rp.dd_func_lags(panel,ll,ll.GAR_1MA, ll.h_2z_val,						g.dLL_lnv) 
		self.d2lnv_z_rho				=	rp.dd_func_lags(panel,ll,ll.GAR_1MA, rp.prod((ll.h_ez_val,g.de_rho)),	g.dLL_lnv) 
		self.d2lnv_z_lambda				=	rp.dd_func_lags(panel,ll,ll.GAR_1MA, rp.prod((ll.h_ez_val,g.de_lambda)),g.dLL_lnv) 
		self.d2lnv_z_beta				=	rp.dd_func_lags(panel,ll,ll.GAR_1MA, rp.prod((ll.h_ez_val,g.de_beta)),	g.dLL_lnv) 
		
		self.d2lnv_rho2,	self.d2e_rho2	=	rp.dd_func_lags_mult(panel,ll,g,	None,	g.de_rho,		g.de_rho,		'rho',		'rho' )
		self.d2lnv_beta2,self.d2e_beta2	=	rp.dd_func_lags_mult(panel,ll,g,	None,	g.de_beta,		g.de_beta,		'beta',		'beta')



		(self.de_rho_RE,self.de_lambda_RE,self.de_beta_RE)=(g.de_rho_RE,g.de_lambda_RE,g.de_beta_RE)
		(self.dlnv_sigma_rho,self.dlnv_sigma_lambda,self.dlnv_sigma_beta)=(g.dlnv_sigma_rho,g.dlnv_sigma_lambda,g.dlnv_sigma_beta)
		(self.dlnv_gamma,self.dlnv_psi)=(g.dlnv_gamma,g.dlnv_psi)
		(self.dlnv_mu,self.dlnv_z)=(g.dlnv_mu, g.dlnv_z)		
