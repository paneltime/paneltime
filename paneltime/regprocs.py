...
def dd_func_lags_mult(panel,ll,g_obj,AMAL,de_xi,de_zeta,vname1,vname2,transpose=False, de_zeta_u=None):
	...
	(N,T,m)=de_xi.shape
	(N,T,k)=de_zeta.shape
	DLL_e=g_obj.DLL_e.reshape(N,T,1,1)
	u_calc=False
	...
	if panel.m>0:
		...
		d2lnv_zeta_xi=np.sum(np.sum(d2lnv_zeta_xi*g_obj.dLL_lnv.reshape((N,T,1,1)),0),0)
	else:
		d2lnv_zeta_xi=None
