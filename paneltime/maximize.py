...
def lnsrch(ll, direction,mp,its,incr,po,prev_dx):
	args=ll.args.args_v
	g = direction.g
	dx = direction.dx
	...
	ll,msg,lmbda,ok=lnsrch_master(args, dx,panel,constr,mp,LL0,ll)
	...
	if not ok and False:
		...
		ll,msg,lmbda,ok=lnsrch_master(args, dx,panel,constr,mp,LL0)
		if not ok:
			dx=ll.args.args_v*(np.arange(len(g))==i)
			ll,msg,lmbda,ok=lnsrch_master(args, dx,panel,constr,mp,LL0)			
...
def lnsrch_master(args, dx,panel,constr,mp,LL0,ll):
...
def maximize(panel,direction,mp,args,tab,msg_main=""):
	...
	its, k, m, dx_norm,incr		=0,  [1,False,-1,0],     0,    None, 0
	...
	while 1:
		...
		direction.get(ll,its,msg,incr)
		LL0=ll.LL
		#Convergence test:
		conv=convergence_test(direction, convergence_limit, its, args_archive, min_iter, b,ll,panel,lmbda,k,incr)
		if conv: 
			printout_func(1.0,"Convergence on zero gradient; maximum identified",ll,its,direction,incr,po,3,'done')
			return ll,direction,po
		if lmbda==0:
			direction.non_convergence=True
		...
		ll,msg,lmbda,ok=lnsrch(ll,direction,mp,its,incr,po,prev_dx)
		incr=ll.LL-LL0
		if not printout_func(1.0,msg,ll,its,direction,incr,po,0):return ll,direction,po
		its+=1
		
def convergence_test(direction,convergence_limit,its,args_archive,min_iter,b,ll,panel,lmbda,k,incr):
	if incr>0:
		return False
	max_dx=max(np.abs(direction.dx_norm))
	constr_conv=max_dx<convergence_limit*k[0]
	unconstr_conv=np.max(np.abs(direction.dx_unconstr)) < convergence_limit*k[0]
